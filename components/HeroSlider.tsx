...
export const HeroSlider: React.FC<HeroSliderProps> = ({ posts }) => {
  ...
  return (
    <div className="relative w-full h-[280px] md:h-[520px] bg-gray-900 overflow-hidden group">
      {/* Background Image */}
      ...
      <button 
        ...
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/60 text-white p-2 rounded-full backdrop-blur-sm transition-all opacity-60 hover:opacity-100"
      >
        <ChevronLeft size={32} />
      </button>
      <button 
        onClick={() => setCurrentIndex((prev) => (prev + 1) % featuredPosts.length)}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/60 text-white p-2 rounded-full backdrop-blur-sm transition-all opacity-60 hover:opacity-100"
      >
